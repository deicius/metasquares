...
function takeTextboxValue(){
  for(let i=0; i<6; i++){
    for(let j=0; j<6; j++){
      board[i][j] = textbox.value().charCodeAt(i*6+j)-48;
    }
  }
}
...
function inb(a) {
  return 0 <= int(a) && int(a) < board_size;
}
...
function inputbuttonstuff(){
  fill(255,255,255);
  rect(580,300,200,100);
  fill(0,0,0);
  textSize(20);
  text("input button", 630, 350);
  if(mouseX>580 && mouseX<780 && mouseY>300 && mouseY<400){
    if(mouseIsPressed){
      takeTextboxValue();
      calculation();
      gameboard();
